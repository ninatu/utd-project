...
def load_backbone(args, **kwargs):
    model = create_model(
        ...
        use_checkpoint=False,
        enable_last_norm=args.enable_last_norm
    )
    ...
    elif 'module' in checkpoint_model:
        checkpoint_model = checkpoint_model['module']
    else:
        raise ValueError(f"'model' or 'module' is not found in checkpoint! Checkpoint keys are: {checkpoint_model.keys()}")
    ...
    assert missing_keys == []
    # assert unexpected_keys == ['mask_token', 'norm.weight', 'norm.bias', 'encoder_to_decoder.weight'] or \
    #        unexpected_keys == ['fc_norm.weight', 'fc_norm.bias', 'head.weight', 'head.bias']

    # TODO
    # assert unexpected_keys == ['mask_token', 'encoder_to_decoder.weight'] or \
    #        unexpected_keys == ['fc_norm.weight', 'fc_norm.bias', 'head.weight', 'head.bias']
