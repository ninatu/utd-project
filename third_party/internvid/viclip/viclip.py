...
def load_backbone(args):
    vclip = ViCLIP(size=args.model, pretrain=args.backbone_checkpoint)
    return vclip
...
class ViCLIP(nn.Module):
    ...
    def get_vision_embedding_and_features(self, video):
        pooled_vision_embeds = self.encode_vision(video, test=True)
        return None, pooled_vision_embeds

    def get_text_embedding_and_features(self, text):
        pooled_text_embeds = self.encode_text(text)
        return None, pooled_text_embeds
